...
for model in sys.argv[1:]:
    ...
    print(f"\n== {model} L{L}: AUC of held-out pain category (S2 wording, same set as training but category unseen; axis from the other four S2 categories) vs ...")
    ...
    for held in PAIN:
        ...
        OUT.setdefault(model, {})[NAMES[held]] = dict(zip(negs.keys(), [round(float(x), 3) for x in row]))
json.dump(OUT, open(HERE / 'out' / 'loco_specificity_s2wording.json', 'w'), indent=1)
